...
export default function Home() {
  ...
  const handleDropdownChange = async (e: { target: { value: any; }; }) => {
    const selectedValue = e.target.value;
    const response = await axios.get(`http://localhost:5000/source/${selectedValue}`);
        ...
        <div className='w-5/12'>
          ...
          <div className="rounded-lg shadow-md w-full h-1/3">
            <div className='flex items-center justify-center'>
              ...
              <Dropdown className="h-20 px-4" onChange={handleDropdownChange}></Dropdown>
            </div>
            ...
            <div className='flex items-center justify-center py-4 text-xl'>
              <button className={`${recordingStatus ? "bg-red-600 hover:bg-red-500" : "bg-emerald-600 hover:bg-emerald-500"} mx-4 p-6 py-4 rounded-lg font-semibold focus:outline-none text-white`} onClick={record}>{recordingStatus ? "Stop Recording" : "Start Recording"}</button>
              <a href="/database"><button className="p-6 py-4 mx-4 rounded-lg bg-sky-950 font-semibold text-white hover:bg-sky-800">View Database</button></a>
              <a href="http://localhost:8086/"><button className="p-6 py-4 mx-4 rounded-lg bg-sky-950 font-semibold text-white hover:bg-sky-800">Manage Database</button></a>
            </div>
          </div>
        </div>
